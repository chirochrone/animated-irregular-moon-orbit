...
Usage
-----
    python margaret_trajectory.py                            # fetch + animate
    python margaret_trajectory.py --no-fetch                 # reuse cached CSV, re-animate
    python margaret_trajectory.py --cache my.csv             # custom cache filename
    python margaret_trajectory.py --fps 50 --dpi 40          # tweak output
    python margaret_trajectory.py --no-fetch --stride 10      # skip every 10th frame (10× faster)
    python margaret_trajectory.py --no-fetch --figsize 5     # smaller figure (faster render)
    python margaret_trajectory.py --no-fetch --mp4           # also export MP4 via ffmpeg
...
Output
------
    margaret_cache.csv          — raw Horizons vectors (auto-reused on reruns)
    margaret_trajectory.gif     — animated GIF timelapse
    margaret_trajectory.mp4     — MP4 (only with --mp4 and ffmpeg installed)
"""
...
def fetch_horizons(cache_csv: str) -> pd.DataFrame:
    ...
    print("Querying JPL Horizons for Margaret (id=723) …")
    obj = Horizons(
        id="723",                 # Margaret = Uranus XXIII  (correct NAIF id)
        location="500@799",       # origin = Uranus body centre
        epochs={
            "start": "1601-Jan-01",
            "stop":  "2399-Jan-01",
            "step":  "15d",       # 50-day timestep
        },
    )
    tbl = obj.vectors()
...
def make_frame(i: int, df: pd.DataFrame,
    ...
    lbl = ax.text2D(
        0.50, 0.97,
        f"Margaret (Uranus XXIII) - {int(row['year'])}-Jan-01",
...
def main():
    ...
    parser.add_argument("--out",    default="margaret_trajectory.gif",
                        help="Output GIF path (default: margaret_trajectory.gif).")
